...
const Todos = () => {
  ...
  useEffect(async () => {
    const url = "https://still-shore-57627.herokuapp.com/api";
  ...
  const handleSubmit = async (e, id) => {
    ...
    if (buttonText === "Add Todo") {
      try {
        ...
        const url = "https://still-shore-57627.herokuapp.com/api/add";
      ...
      try {
        const load = { todo: input };
        const url = `https://still-shore-57627.herokuapp.com/api/${itemId}`;
        const todoItem = await fetch(url, {
          method: "PUT",
          mode: "cors",
          body: JSON.stringify(load),
